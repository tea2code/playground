...
def render( state, frameTime ):
    print( 'State (', round(state.x, 4), ',', round(state.v, 4), '); Frametime =', '%.10f' % frameTime, 's' )
...
while math.fabs(state.x) > 1 or math.fabs(state.v) > 1:
    ...
    render( state, frameTime )
    time.sleep( 0.3 ) # Simulate high load.